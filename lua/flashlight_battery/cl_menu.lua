...
local function PopulateAbout(cpanel)
    ...
    about:Help("Adds battery drain, recharge, cooldown, and a simple HUD for flashlights.")
    AddLinkButton(about, "GitHub Repository", links.repository)

    local compatibility = AddForm(cpanel, "Compatibility")
    compatibility:Help("Works with the default flashlight and supports Better Flashlight.")
    compatibility:Help("Need another flashlight addon supported? Link the addon and say what is not working.")
    AddLinkButton(compatibility, "Request Compatibility", links.issue)

    local support = AddForm(cpanel, "Support")
    support:Help("Useful details: the addon link, your settings, and whether the battery drains, blocks, or HUD behave incorrectly.")
